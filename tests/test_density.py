...
class TestLambMatrix:
    ...
    def test_lamb_matrix_simple(self):
        ...
        # For a 2x2 matrix, k runs over {0, 1} and we exclude k=n
        # L[0,0]: n=0, so k=1: L = 0.5 * (G[1,0] + G[1,0]) = 0.5 * (2 + 2) = 2.0
        # L[0,1]: n=1, so k=0: L = 0.5 * (G[0,0] + G[0,1]) = 0.5 * (0 + 1) = 0.5
        # L[1,0]: n=0, so k=1: L = 0.5 * (G[1,1] + G[1,0]) = 0.5 * (0 + 2) = 1.0
        # L[1,1]: n=1, so k=0: L = 0.5 * (G[0,1] + G[0,1]) = 0.5 * (1 + 1) = 1.0
        assert L[0, 0] == 2.0
        assert L[0, 1] == 0.5
        assert L[1, 0] == 1.0
        assert L[1, 1] == 1.0
