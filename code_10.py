...
T_0 = 2.094
tlist = np.linspace(0, 10000 * T_0, 25000)
deltas = np.linspace(0, +9*t23, 100)
dt = tlist[1] - tlist[0]
N = len(tlist)
...
for i, d1 in enumerate(deltas):
    ...
    for j, d2 in enumerate(deltas):
        ...
        H = np.array([
            [e1u,                 0,                 -tau_0,                 -t12*exp(-1j*a_12),              -tau_0,                 t31*exp(-1j*a_31)],
            [0,                 e1d,                 t12*exp(1j*a_12),                 -tau_0,                 -t31*exp(1j*a_31),                 -tau_0],
            [-tau_0,    t12*exp(-1j*a_12),                 e2u,                 0,                 -tau_0,                 -t23*exp(-1j*a_23)],
            [-t12*exp(1j*a_12),                 -tau_0,                 0,                 e2d,                 t23*exp(1j*a_23),                 -tau_0],
            [-tau_0,                 -t31*exp(-1j*a_31),                 -tau_0,                 t23*exp(-1j*a_23),                 e3u,                 0],
            [t31*exp(1j*a_31),                 -tau_0,                 -t23*exp(1j*a_23),                 -tau_0,                 0,                 e3d]])
        
        # part 2
        
        U_dt = expm(-1j * H * dt)
        rho_ii_t = evolve_U(U_dt, N, phi_0)
        store_dd[j, i, :] = np.max(rho_ii_t, axis=0)
         """
...
with plt.style.context(['science']):
    ...
    for i in range(5):
        ...
        im = axs[i].imshow(store_dd[:, :, i+1], origin='lower', cmap='viridis', vmin=0, vmax=1, extent=[0, 9, 0, 9])
        axs[i].set_xlabel(r'$\tau_{12}[\tau_0]$', fontsize=fontsize)
        axs[i].set_ylabel(r'$\tau_{31}[\tau_0]$', fontsize=fontsize)
        # axis equal
        axs[i].set_aspect('equal')
        plt.colorbar(im, ax=axs[i])

    # tight layout without changing axes size and ticks
    plt.subplots_adjust(left=None, bottom=None, right=None, top=None, wspace=0.6, hspace=0.1)
